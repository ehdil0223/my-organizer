...
function renderPending() { $('#pending-files').innerHTML = state.pending.map((file, i) => `<span class="file-chip">${isImage(file) ? `<img class="file-thumb" src="${URL.createObjectURL(file)}" alt="" />` : ''}${escapeHtml(file.name)} <button type="button" data-remove="${i}" aria-label="파일 제거">×</button></span>`).join(''); document.querySelectorAll('[data-remove]').forEach((button) => button.onclick = () => { state.pending.splice(Number(button.dataset.remove), 1); renderPending(); }); }
...
$('#save-button').onclick = async () => {
  ...
  try { const newFiles = state.pending.length ? await uploadFiles(id) : []; const files = [...state.existingFiles, ...newFiles]; const payload = { id, title, content, files, updated_at:new Date().toISOString() }; if (!state.editingId) payload.created_at = payload.updated_at;
    const { error } = await supabaseClient.from('records').upsert(payload); if (error) throw error; clearComposer(); toast(old ? '수정했어요.' : '저장했어요.'); await loadRecords();
  } catch { toast('저장에 실패했어요. Supabase 설정을 확인해 주세요.'); } finally { $('#save-button').disabled=false; $('#save-button').textContent='기록 저장'; }
};
async function uploadFiles(recordId) { const uploaded = []; for (const file of state.pending) { const path = `shared/${recordId}/${crypto.randomUUID()}-${file.name}`; const { error } = await supabaseClient.storage.from('record-files').upload(path, file); if (error) throw error; uploaded.push({ name:file.name, path, type:file.type }); } return uploaded; }
async function loadRecords() { const { data, error } = await supabaseClient.from('records').select('*').order('updated_at', { ascending:false }); if (error) return toast('기록을 불러오지 못했어요.'); state.records = data; await render(); }
async function render() { const list=$('#record-list'); list.innerHTML=''; $('#empty-state').hidden=state.records.length>0; $('#record-count').textContent=state.records.length?`${state.records.length}개`:''; for (const record of state.records) { const node=$('#record-template').content.cloneNode(true); node.querySelector('.record-meta').textContent=new Date(record.updated_at).toLocaleString('ko-KR',{dateStyle:'medium',timeStyle:'short'}); node.querySelector('h3').textContent=record.title; node.querySelector('.record-content').textContent=record.content;
  const files=node.querySelector('.attachments'); for (const file of record.files || []) { const { data }=await supabaseClient.storage.from('record-files').createSignedUrl(file.path, 3600); if(data){const link=document.createElement('a');link.className='file-chip';link.href=data.signedUrl;link.target='_blank'; if(isImage(file)){const image=document.createElement('img');image.className='file-thumb';image.src=data.signedUrl;image.alt=file.name;link.append(image);} link.append(`📎 ${file.name}`);files.append(link);} }
...
async function remove(record) { if(!confirm(`“${record.title}” 기록을 삭제할까요?`)) return; const paths=(record.files||[]).map((file)=>file.path); if(paths.length) await supabaseClient.storage.from('record-files').remove(paths); const { error }=await supabaseClient.from('records').delete().eq('id',record.id); if(error) return toast('삭제하지 못했어요.'); await loadRecords(); toast('삭제했어요.'); }
...
function isImage(file) { return file.type?.startsWith('image/') || /\.(png|jpe?g|gif|webp|heic)$/i.test(file.name); }
async function startApp() { await loadRecords(); supabaseClient.channel('shared-records').on('postgres_changes',{event:'*',schema:'public',table:'records'},loadRecords).subscribe(); }
